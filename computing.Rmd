---
title: "Accessing and Using Rollins HPC: A Rudimentary Introduction"
affiliation: Department of Epidemiology, Emory University
output: html_document
---

#### Introduction: The Center for Resource Computing

The Rollins School of Public Health houses a computing cluster that our group has access to. To gain access you must first open an account, and acquire access to the Emory-VPN HIPAA core, which can be done by following the instructions here:
...
The cluster is a unix server that can be accessed from the command line on your computer (e.g., Terminal on macOS). First, ensure you are connected to the VPN. Next, open up the command line interface. This includes Terminal on a Mac (or a Terminal emulator, such as Iterm2), Terminal on a Linux machine, or Powershell on a Windows 10 Machine. Once open, you are interacting with the command line interface (CLI), or the prompt from which you can access the cluster. To access the cluster, you must login via SSH which can be done with the following command:

```{}
ssh anaimi@clogin01.sph.emory.edu
```

where my user name (anaimi) will be replaced with yours. If you enter the above on the command line (and are connected to the VPN), you should be prompted to enter a password. If it's your first time logging on, you will need to follow the instructions in the above user's guide. Otherwise, use the password you set at your first login and proceed. This should connect you to the cluster. You will know that you have successfully connected when you see the welcome page on your command line interface:
...
%There are a lot of different options available to a user. More detailed information can be found on the CRC website. To get a sense of how jobs are prioritized in the queue (and thus how you should optimize these options so as to not wait too long for your job to start), refer to the Fair Share and Priority section [here](https://crc.pitt.edu/h2p).

The next section in the shell script tells the cluster which modules you need to run your job. It's important to start this section with `module purge`, which clears up the environment, thus avoiding potential conflict between modules. %You will then have to load any modules you may need, which may include compilers such as `module load gcc` (you will need this and others, especially if you use the tidyverse or Rcpp). 
